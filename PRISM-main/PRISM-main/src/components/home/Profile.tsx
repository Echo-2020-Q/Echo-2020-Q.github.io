...
interface ProfileProps {
    ...
    visitorMap?: SiteConfig['visitor_map'];
    researchInterests?: string[];
}

function VisitorMap({ visitorMap }: { visitorMap?: SiteConfig['visitor_map'] }) {
    const scriptContainerRef = useRef<HTMLDivElement>(null);
    const [scriptFailed, setScriptFailed] = useState(false);
    const enabled = visitorMap?.enabled;
    const title = visitorMap?.title || 'Visitor Map';
    const note = visitorMap?.note?.trim();
    const scriptId = visitorMap?.script_id?.trim() || 'clustrmaps';
    const scriptUrl = visitorMap?.script_url?.trim();
    const imageUrl = visitorMap?.image_url?.trim();
    const linkUrl = visitorMap?.link_url?.trim();

    useEffect(() => {
        const container = scriptContainerRef.current;

        if (!enabled || !scriptUrl || !container) return;
    ...
    }, [enabled, scriptId, scriptUrl]);

    if (!enabled) return null;
    if (!scriptUrl && !imageUrl) return null;

    const imageMap = imageUrl ? (
    ...
    return (
        <div className="mt-6 rounded-lg bg-neutral-100 p-4 shadow-sm transition-all duration-200 hover:shadow-lg dark:bg-neutral-800">
            <h3 className="mb-3 text-center font-semibold text-primary">{title}</h3>
            ...
            {note && (
                <p className="mt-1 text-center text-xs leading-relaxed text-neutral-500 dark:text-neutral-500">
                    {note}
                </p>
            )}
        </div>
...
export default function Profile({ author, social, features, visitorMap, researchInterests }: ProfileProps) {
    ...
    const [hoveredSocialName, setHoveredSocialName] = useState<string | null>(null);
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('yihang-website-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('yihang-website-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('yihang-website-user-liked');
            setShowThanks(false);
        }
    };
        ...
        >
                    ...
                    return (
                        <div key={link.name} className="relative">
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                onMouseEnter={() => setHoveredSocialName(link.name)}
                                onMouseLeave={() => setHoveredSocialName(null)}
                                onFocus={() => setHoveredSocialName(link.name)}
                                onBlur={() => setHoveredSocialName(null)}
                            ...
                            <AnimatePresence>
                                {hoveredSocialName === link.name && (
                                    <motion.div
                                        initial={{ opacity: 0, y: 6, scale: 0.95 }}
                                        animate={{ opacity: 1, y: -6, scale: 1 }}
                                        exit={{ opacity: 0, y: 0, scale: 0.95 }}
                                        className="pointer-events-none absolute top-0 left-1/2 z-10 -translate-x-1/2 -translate-y-full whitespace-nowrap rounded-md bg-neutral-800 px-2.5 py-1.5 text-xs font-medium text-white shadow-lg"
                                    >
                                        {link.name}
            ...
            {researchInterests && researchInterests.length > 0 && (
                <div className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-4 mb-6 hover:shadow-lg transition-all duration-200 hover:scale-[1.02]">
                    ...
                    <div className="space-y-2 text-center text-sm text-neutral-700 dark:text-neutral-500">
